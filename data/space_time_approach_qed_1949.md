...
, the associated quantity is written in Feynman's shorthand as 
...
, and it depends on only the momentum and polarization of the photon. The similar quantity for an electron moving from 
...
. It depends on the momentum and polarization of the electron, in addition to a constant Feynman calls n, sometimes called the "bare" mass of the electron: it is related to, but not the same as, the measured electron mass. Finally, the quantity that tells us about the probability amplitude for an electron to emit or absorb a photon Feynman calls j, and is sometimes called the "bare" charge of the electron: it is a constant, and is related to, but not the same as, the measured electron charge e.[2]: 91
...
{\displaystyle x_{A}}
 ...
 stands for the four real numbers that give the time and position in three dimensions of the point labeled A.
...
The QED Lagrangian for a spin-1/2 field interacting with the electromagnetic field in natural units gives rise to the action[27]: 78
...
{\displaystyle {\text{U}}(1)}
 ...
 current arising from Noether's theorem. It is written
...
{\displaystyle B_{\mu }}
 ...
 has been set to zero, with no loss of generality. Alternatively, we can absorb 
...
{\displaystyle A'_{\mu }=A_{\mu }+B_{\mu }}

 and relabel the new field as 
...
From this Lagrangian, the equations of motion for the 
...
These arise most straightforwardly by considering the Euler-Lagrange equation for 
...
. Since the Lagrangian contains no 
...
{\displaystyle \partial _{\mu }{\bar {\psi }}}

 terms, we immediately get
...
{\displaystyle (i\gamma ^{\mu }\partial _{\mu }-m)\psi =e\gamma ^{\mu }A_{\mu }\psi .}
...
{\displaystyle S_{fi}=\langle f|U|i\rangle .}

This technique is also known as the S-matrix, and 
...
{\displaystyle S_{fi}}
 ...
 is therefore known as the S-matrix element. The evolution operator is obtained in the interaction picture, where time evolution is given by the interaction Hamiltonian, which is the integral over space of the second term in the Lagrangian density given above:[27]: 123
...
Which can also be written in terms of an integral over the interaction Hamiltonian density 
...
{\displaystyle {\mathcal {H}}_{I}=e{\overline {\psi }}\gamma ^{\mu }\psi A_{\mu }}
...
where T is the time-ordering operator. This evolution operator only has meaning as a series, and what we get here is a perturbation series with the fine-structure constant as the development parameter. This series expansion of the probability amplitude 
...
{\displaystyle S_{fi}}
 ...
 is called the Dyson series, and is given by:
...
{\displaystyle S_{fi}=\langle f|U|i\rangle =\left\langle f\left|\sum _{n=0}^{\infty }{\frac {(-i)^{n}}{n!}}\int d^{4}x_{1}\cdots \int d^{4}x_{n}T{\bigg \{}{\mathcal {H}}(x_{1})\cdots {\mathcal {H}}(x_{n}){\bigg \}}\right|i\right\rangle }
...
{\displaystyle S_{fi}}
 ...
 typically contains delta functions that are not physically-measurable, it is convenient to define the invariant amplitude 
...
{\displaystyle S_{fi}=\langle f|i\rangle +(iM_{fi}\times {\text{Delta function terms}})}
...
{\textstyle \int d^{4}p/(2\pi )^{4}}

, since these internal ("virtual") particles are not constrained to any specific energy–momentum, even that usually required by special relativity (see Propagator for details). The signature of the metric 
